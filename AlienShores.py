...
conv = True
svol = 50 if conv else 70
bvol = 90 if conv else 95
lvol1 = 85 if conv else 85
lvol2 = 85 if conv else 85
rvol = 110 if conv else 110
wvol = 70 if conv else 100
cvol = 127 if conv else 110
if not conv:
  officialName += " for midiplayer"

instrS = mkInstrument(122,0,70)
instrB = mkInstrument(44,5,95)
instrL = mkInstrument( \
	[ (88,4,85), (4,4,85) ] )
instrR = mkInstrument(5,5,110)
instrW = mkInstrument(78,5,100)
instrC = mkInstrument(52,5,110)

bass = song.addTrack(instrB)
left = song.addTrack(instrL)
right = song.addTrack(instrR)
numWhistles = 2
whistles = []
for i in range(0,numWhistles):
    whistles.append(song.addTrack(instrW))
numChoir = 4
choir = []
for i in range(0,numChoir):
    choir.append(song.addTrack(instrC))
shore = song.addTrack(instrS)
