...
class PlayerBase:
  ...
  def writeMelody(self,midiw,instr):
    ...
    self.writeNotes(midiw,instr,self.melody.notes)
    for gs in self.melody.gradients:
  ...
  def write(self,midiw):
    self.melody.sort()
    if not self.melody.choirized:
      self.melody.deinterleave()
